...
The `normpic/manager/photo_manager.py` module provides complete photo collection organization workflow orchestration.
...
2. Extracts EXIF and camera metadata via `normpic/util/exif.py`
3. Orders photos with burst preservation via `_order_photos_with_burst_preservation()`
4. Generates standardized filenames via `normpic/template/filename.py`
5. Creates symlinks in destination directory
6. Generates and saves JSON manifest via `normpic/serializer/manifest.py`
...
- `normpic/util/exif.py` - EXIF extraction
- `normpic/template/filename.py` - Filename generation  
- `normpic/serializer/manifest.py` - JSON serialization
- `normpic/model/manifest.py` - Data structures