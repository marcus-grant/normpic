...
class ManifestManager:
    ...
    def compute_file_hash(self, file_path: Union[Path, str]) -> str:
        ...
        """
        try:
            return content_id(Path(file_path).read_bytes())
    ...
    def needs_reprocessing(
        ...
        if previous_mtime is not None:
            ...
            if datetime.fromtimestamp(current_mtime, tz=timezone.utc).strftime(
                _fmt
    ...
    def needs_reprocessing_by_hash(
        ...
        current_iso = datetime.fromtimestamp(current_mtime, tz=timezone.utc).strftime(
            _fmt
        )
        prev_iso = datetime.fromtimestamp(prev_mtime_float, tz=timezone.utc).strftime(
            _fmt
        )
    ...
    def config_affects_reprocessing(
    ...
    def destination_file_missing(
        self, source_path: Union[Path, str], dest_path: Union[Path, str]
    ) -> bool:
