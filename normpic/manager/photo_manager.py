...
def organize_photos(
    ...
    hash_index = (
        build_hash_keyed_source_index(existing_manifest) if existing_manifest else {}
    )
            ...
            if (
                ...
                try:
                    ...
                    error_handler.handle_exif_extraction_failure(
                        photo_path, str(e), fallback_timestamp=fallback_timestamp
                    )
                    # Add with fallback data
                    pics_with_metadata.append((photo_path, None, None))
                except Exception:
                    # Complete failure - skip this file
                    error_handler.handle_filesystem_error(
                        photo_path, f"Cannot access file: {e}"
                    )
            continue
...
def _order_photos_with_burst_preservation(pics_data):
    ...
    while i < len(temporally_sorted):
        ...
        while j < len(temporally_sorted):
            ...
            if (
                next_exif_data.timestamp
                and next_exif_data.timestamp.replace(microsecond=0) == main_timestamp
            ):
        ...
        if len(cameras_in_group) <= 1:
            ...
            for cam_key, cam_pics in sorted(
                camera_groups.items(), key=camera_earliest_sort_key
            ):
...
def _create_ordered_pics(pics_data, collection_name: str, dest_dir: Path) -> List[Pic]:
    ...
    pics: list[Pic | None] = [None] * len(pics_data)  # Oredered pre-allocation

    for group_key, group_photos in burst_groups.items():
        base_filename = ""  # pre-allocation for the filename, mostly to shutup pyright
        dest_filename = ""
        ...
        for j, (i, photo_path, exif_data, camera_info) in enumerate(group_photos):
            ...
            pic = Pic(
                ...
                gps={"lat": exif_data.gps_latitude, "lon": exif_data.gps_longitude}
...
def resolve_symlink_pairs_by_hash(
    ...
    for pic in copy_pics:
        ...
        if source_pic.relative_path is None:
            raise RuntimeError(f"source pic has no relative_path: {source_pic.hash}")
        source_resolved = (
