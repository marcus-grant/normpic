...
class ErrorHandler:
    ...
    def handle_corrupted_file(self, file_path: Path, details: str) -> ErrorEntry:
        ...
        entry = ErrorEntry(
            error_type="corrupted_file", source_file=file_path.name, details=details
        )
    ...
    def handle_exif_extraction_failure(
        self, file_path: Path, details: str, fallback_timestamp: Optional[str] = None
    ) -> ErrorEntry:
        ...
        entry = ErrorEntry(
            error_type="exif_error", source_file=file_path.name, details=details_text
        )
        self.warnings.append(entry)
    ...
    def handle_filesystem_error(self, file_path: Path, details: str) -> ErrorEntry:
        ...
        entry = ErrorEntry(
            error_type="filesystem_error", source_file=file_path.name, details=details
        )
    ...
    def add_warning(self, error_type: str, file_path: Path, details: str) -> ErrorEntry:
        ...
        entry = ErrorEntry(
            error_type=error_type, source_file=file_path.name, details=details
        )
        self.warnings.append(entry)
        self._files_skipped += 1
        return entry
    ...
    def get_processing_summary(self) -> Dict[str, Any]:
        ...
        files_processed_successfully = (
            total_files - self._files_skipped if not self.has_blocking_errors() else 0
        )
