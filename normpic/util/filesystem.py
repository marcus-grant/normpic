...
def detect_broken_symlinks(
    directory: Path, progress_callback: Optional[Callable[[int, int], None]] = None
) -> List[Path]:
...
def compute_file_hash(
    ...
    """Compute the NormPic content id of a file.

    Reads the whole file into memory and delegates to the b3c32-backed
    content_id wrapper.
    chunk_size and progress_callback are accepted but unused: streaming
    is deferred to v0.2, pending a b3c32 streaming entry point.

    Args:
        file_path: Path to the file to hash.
        chunk_size: Unused; reserved for the v0.2 streaming path.
        progress_callback: Unused; reserved for the v0.2 streaming path.

    Returns:
        The b3-120: prefixed content id.
    ...
    return content_id(file_path.read_bytes())


def scan_directory_symlinks(
    directory: Path, progress_callback: Optional[Callable[[str, int, int], None]] = None
) -> dict[str, List[Path]]:
    ...
    """
    results = {"valid": [], "broken": [], "regular_files": [], "directories": []}
...
def batch_validate_symlinks(
    symlinks: List[Path], progress_callback: Optional[Callable[[int, int], None]] = None
) -> dict[Path, bool]:
