...
class TestComputeFileHash:
    ...
    def test_compute_file_hash_returns_valid_content_id(self):
        """compute_file_hash returns a well-formed b3-120 content id."""
        with tempfile.TemporaryDirectory() as temp_dir:
            temp_path = Path(temp_dir)
            test_file = temp_path / "test.jpg"
            test_file.write_bytes(b"test data")
            hash_result = compute_file_hash(test_file)
            assert_valid_content_id(hash_result)
    ...
    def test_compute_file_hash_with_mock(self):
        """Hash computation with mocked file I/O returns a valid id."""
        fake_path = Path("/fake/path.jpg")
        with (
            patch.object(Path, "exists", return_value=True),
            patch.object(Path, "read_bytes", return_value=b"mocked data"),
        ):
            hash_result = compute_file_hash(fake_path)
            assert_valid_content_id(hash_result)

    @pytest.mark.skip(
        reason="chunked reads deferred to v0.2; awaiting b3c32 streaming API"
    )
    def test_compute_file_hash_custom_chunk_size(self):
        """Hashing with a custom chunk size (deferred to v0.2).

        compute_file_hash reads the whole file and delegates to the
        b3c32-backed wrapper, which exposes no chunk-size control.
        When b3c32 gains a streaming API, restore this to assert a small
        chunk_size yields the same content id as the default.
        """
        with tempfile.TemporaryDirectory() as temp_dir:
            temp_path = Path(temp_dir)
            test_file = temp_path / "test.jpg"
    ...
    @pytest.mark.skip(
        reason="progress callback deferred to v0.2; awaiting b3c32 streaming API"
    )
    def test_compute_file_hash_progress_callback(self):
        """Progress callback during hashing (deferred to v0.2).

        The whole-file delegation has no chunk loop to report progress
        from.
        When b3c32 gains a streaming API, restore this to assert the
        callback fires with cumulative and total byte counts.
        """
        progress_updates = []
    ...
    def test_detect_broken_symlinks_with_progress(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            broken = detect_broken_symlinks(
                temp_path, progress_callback=progress_callback
            )
...
class TestScanDirectorySymlinks:
    ...
    def test_scan_directory_symlinks_with_progress(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            results = scan_directory_symlinks(
                temp_path, progress_callback=progress_callback
            )
    ...
    def test_batch_validate_symlinks_with_progress(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            results = batch_validate_symlinks(
                symlinks, progress_callback=progress_callback
            )
