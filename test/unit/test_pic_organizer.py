...
from lib.manager.photo_manager import (
...
class TestDeterminePicOrder:
    ...
    def test_exif_timestamp_precedence(self):
        ...
        late_exif = ExifData(
            timestamp=datetime(2024, 10, 5, 14, 30, 47), 
            subsecond=456
        )
        
        pics_data = [
            (late_photo, late_exif, CameraInfo()),  # Later timestamp, earlier filename
            (early_photo, early_exif, CameraInfo())  # Earlier timestamp, later filename  
        ]
...
class TestGroupByBurstSequence:
    ...
    def test_single_camera_burst_stays_together(self):
        ...
        canon_pics = [
            (Path("IMG_001.jpg"), ExifData(timestamp=base_time, subsecond=100), canon_camera_info),
            (Path("IMG_002.jpg"), ExifData(timestamp=base_time, subsecond=200), canon_camera_info), 
            (Path("IMG_003.jpg"), ExifData(timestamp=base_time, subsecond=300), canon_camera_info)
        ]
    ...
    def test_different_cameras_separate_bursts(self):
        ...
        mixed_pics = [
            (Path("IMG_001.jpg"), ExifData(timestamp=base_time, subsecond=100), canon_camera_info),
            (Path("IMG_3456.heic"), ExifData(timestamp=base_time, subsecond=150), iphone_camera_info),
            (Path("IMG_002.jpg"), ExifData(timestamp=base_time, subsecond=200), canon_camera_info),
            (Path("IMG_3457.heic"), ExifData(timestamp=base_time, subsecond=250), iphone_camera_info)
        ]
...
class TestCreateOrderedPics:
    ...
    def test_create_pics_with_burst_counters(self):
        ...
        pics_data = [
            (source_dir / "IMG_001.jpg", ExifData(timestamp=base_time, subsecond=100), camera_info),
            (source_dir / "IMG_002.jpg", ExifData(timestamp=base_time, subsecond=200), camera_info), 
            (source_dir / "IMG_003.jpg", ExifData(timestamp=base_time, subsecond=300), camera_info)
        ]
