...
class MockPath:
    ...
    def readlink(self):
        ...
        msg = "This stupid mock class needs to go and is fragile as hell"
        assert target_path is not None, msg
...
class MockFilesystem:
    ...
    def create_file(self, path: str, content: bytes = b"", size: int | None = None):
        """Create a mock file."""
        if size is None:
            size = len(content)
    ...
    def create_symlink(self, path: str, target: str):
        ...
        self.entries[path] = {"type": "symlink", "target": target}
...
class TestFilesystemWithMocks:
    ...
    def test_create_symlink_with_mock_filesystem(self):
        ...
        # Act
        create_symlink(cast(Path, source), cast(Path, dest), atomic=False)
    ...
    def test_validate_symlink_with_mock_filesystem(self):
        ...
        # Act & Assert
        assert validate_symlink_integrity(cast(Path, link)) is True

        # Test broken symlink
        broken_link = self.fs.path("/broken.jpg")
        broken_link.symlink_to(self.fs.path("/nonexistent.jpg"))

        assert validate_symlink_integrity(cast(Path, broken_link)) is False
    ...
    def test_detect_broken_symlinks_with_mock_filesystem(self):
        ...
        # Act
        broken_links = detect_broken_symlinks(cast(Path, test_dir))

        # Assert
        assert len(broken_links) == 1
        assert broken_links[0].path == broken_link.path  # type:ignore[attr-defiend]
    ...
    def test_compute_file_hash_with_mock_filesystem(self):
        ...
        with patch("builtins.open", mock_open(read_data=test_data)):
            # Act
            hash_result = compute_file_hash(cast(Path, file_path))
    ...
    def test_atomic_symlink_creation_with_mock_filesystem(self):
        ...
        with patch("os.getpid", return_value=12345):
            # Act
            create_symlink(cast(Path, source), cast(Path, dest), atomic=True)
    ...
    def test_progress_callback_functionality(self):
        ...
        # Act
        create_symlink(
            cast(Path, source), cast(Path, dest), progress_callback=progress_callback
        )
