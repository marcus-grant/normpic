...
def run_normpic_with_monitoring(
    config_file: Path, dry_run: bool = False
) -> PerformanceMetrics:
    ...
    msg = "monitor unset after monitoring block; construction failed"
    assert monitor is not None, msg
...
def analyze_manifest(manifest_path: Path) -> Dict[str, Any]:
    ...
    try:
        ...
        analysis = {
            ...
            "timestamp_sources": {},
            "file_extensions": {},
            "size_distribution": {"min_mb": 0, "max_mb": 0, "avg_mb": 0},
        }

        # Analyze timestamp sources
        for pic in pics:
            source = pic.get("timestamp_source", "Unknown")
            analysis["timestamp_sources"][source] = (
                analysis["timestamp_sources"].get(source, 0) + 1
            )
        ...
        # Analyze file sizes
        sizes_mb = [
            pic.get("size_bytes", 0) / (1024 * 1024)
            for pic in pics
            if pic.get("size_bytes")
        ]
        if sizes_mb:
...
def print_performance_summary(
    result: BenchmarkResult, manifest_analysis: Dict[str, Any]
):
    ...
    mb_per_sec = (
        (perf.total_source_size_gb * 1024) / perf.total_time_seconds
    ...
    if manifest_analysis and "error" not in manifest_analysis:
        ...
        if manifest_analysis["timestamp_sources"]:
            print(
                f"  Timestamp Sources: {', '.join(manifest_analysis['timestamp_sources'].keys())}"
            )
